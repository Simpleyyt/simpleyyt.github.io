...
大家都知道我们常用的 `SpringBoot` 项目最终在线上运行的时候都是通过启动 `java -jar xxx.jar` 命令来运行的，那你有没有想过一个问题，那就是当我们执行 `java -jar` 命令后，到底底层做了什么就启动了我们的 `SpringBoot` 应用呢？或者说一个 `SpringBoot` 的应用到底是如何运行起来的呢？今天阿粉就带大家来看下。
...
![](https://tva1.sinaimg.cn/large/008vxvgGgy1h8tb6f2rzgj31k30u0adi.jpg)
...
![](https://tva1.sinaimg.cn/large/008vxvgGgy1h8tb5kvbzxj31lz0u0dlm.jpg)
...
![](https://tva1.sinaimg.cn/large/008vxvgGgy1h8tbb5t73gj329u0oe7ab.jpg)
...
![](https://tva1.sinaimg.cn/large/008vxvgGgy1h8uf3562xjj30q80d6ta1.jpg)
...
![](https://tva1.sinaimg.cn/large/008vxvgGgy1h8ufdvrfj9j31zy0jun2r.jpg)
...
![](https://tva1.sinaimg.cn/large/008vxvgGgy1h8ug0m2bm8j31fa0u00yc.jpg)

跟着代码我们可以看到最终调用的是这个 `run()` 方法

![](https://tva1.sinaimg.cn/large/008vxvgGgy1h8ug5g9iy3j31tg0quwjw.jpg)

![](https://tva1.sinaimg.cn/large/008vxvgGgy1h8ug3v1bscj31yw0mqgqa.jpg)

而这里的参数 `mainClass` 和 `launchClass` 都是通过通过下面的逻辑获取的，都是通过资源文件里面的 `Start-Class` 来进行获取的，这里正是我们项目的启动类，由此可以看到我们上面的猜想是正确的。

![](https://tva1.sinaimg.cn/large/008vxvgGgy1h8ug6ygtpaj31i20emjth.jpg)
